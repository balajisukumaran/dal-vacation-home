...
const updateTicket = async (event) => {
    try {
        ...
        if (!ticketId || !updatedFields) {
            return {
                statusCode: 400,
                body: JSON.stringify({ message: "Missing required fields: ticketId or updatedFields." })
            };
        }
        ...
        if (!status || !comments) {
            return {
                statusCode: 400,
                body: JSON.stringify({ message: "Missing one or more required parameters in updatedFields: status, comments." })
            };
        }
        ...
        return {
            statusCode: 500,
            body: JSON.stringify({ message: "Error updating ticket", error: error.message })
        };
    }
};
